"""
AetherIQ — VIIRS Active Fire Data Download
============================================
Downloads monthly fire-occurrence composites from VIIRS (VNP14A1)
...
VIIRS_COLLECTION: str = "NASA/VIIRS/002/VNP14A1"


def _to_binary_fire(image):
    """Convert VIIRS FireMask to binary fire presence.
...
def download_single_month(
    ...
    """Download a single monthly VIIRS fire-occurrence composite.
    ...
    filename = f"viirs_fire_{year:04d}_{month:02d}.tif"
    out_path = VIIRS_FIRE_DIR / filename
    ...
    if success and verify_download(out_path):
        ...
        save_metadata(
            ...
            year=year,
            month=month,
            filename=f"viirs_fire/{filename}",
...
def download_all_viirs_fire(
    start_date: str = START_DATE,
    end_date: str = END_DATE,
) -> Dict[str, int]:
    """Download all monthly VIIRS fire composites.
    ...
    logger.info(
        "Downloading VIIRS Fire | %s | %d months",
        VIIRS_COLLECTION, len(intervals),
    )

    for m_start, m_end, yr, mo in tqdm(
        intervals, desc="VIIRS Fire", unit="month"
    ):
        try:
            ...
            logger.error(
                "Unhandled error for VIIRS Fire %04d-%02d: %s", yr, mo, exc
            )
            stats["failed"] += 1

    logger.info(
        "VIIRS Fire complete: %d success, %d failed",
