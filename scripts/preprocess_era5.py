"""
AetherIQ — ERA5 Preprocessing
=============================
Processes raw ERA5 GeoTIFFs to replace NoData values and
...
def preprocess_era5(logger: logging.Logger) -> Dict[str, int]:
    """Preprocess all ERA5 files.
    ...
    """
    logger.info("Starting ERA5 preprocessing...")
    
    in_dir = config.ERA5_DIR
    if not in_dir.exists():
        logger.warning("Input directory not found: %s. Skipping.", in_dir)
        return {"processed": 0, "skipped": 0, "failed": 0}
        
    out_dir = config.PROCESSED_ERA5_DIR
    out_dir.mkdir(parents=True, exist_ok=True)
    
    files: List[Path] = sorted(in_dir.glob("*.tif"))
    if not files:
        logger.info("No files found in %s.", in_dir)
        return {"processed": 0, "skipped": 0, "failed": 0}
    ...
    for file_path in tqdm(files, desc="ERA5", unit="file"):
    ...
    logger.info("Finished ERA5 preprocessing.")
    logger.info(
...
def main() -> None:
    """Run the ERA5 preprocessing pipeline.
    ...
    """
    logger = utils.setup_logging("preprocess_era5")
    logger.info("Starting ERA5 preprocessing pipeline...")
    
    pipeline_start_time = time.time()
    
    counts = preprocess_era5(logger)
    ...
    summary = (
        "\n==================================\n"
        "ERA5 Preprocessing Summary\n"
        "==================================\n"
        f"Files processed: {counts['processed']}\n"
        f"Files skipped: {counts['skipped']}\n"
        f"Files failed: {counts['failed']}\n"
