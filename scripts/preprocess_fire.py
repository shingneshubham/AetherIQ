"""
AetherIQ — Fire Preprocessing
=============================
Processes raw MODIS and VIIRS Fire GeoTIFFs to replace NoData values and
...
def preprocess_dataset(
    ...
    """Preprocess all files for a specific fire dataset.
    
    Args:
        input_dir: Directory containing raw GeoTIFFs.
        output_dir: Directory to save processed GeoTIFFs.
        dataset_name: Display name of the dataset (e.g., 'MODIS', 'VIIRS').
...
def main() -> None:
    """Run the Fire preprocessing pipeline.
    
    Reads raw GeoTIFFs for MODIS and VIIRS, replaces NoData values, and saves them
    to the processed directory while maintaining success/failed statistics.
    """
    logger = utils.setup_logging("preprocess_fire")
    logger.info("Starting Fire preprocessing pipeline...")
    
    pipeline_start_time = time.time()
    
    # MODIS
    modis_out = config.PROCESSED_FIRE_DIR / "modis"
    modis_counts = preprocess_dataset(
        input_dir=config.MODIS_FIRE_DIR,
        output_dir=modis_out,
        dataset_name="MODIS",
        logger=logger,
    )
    
    # VIIRS
    viirs_out = config.PROCESSED_FIRE_DIR / "viirs"
    viirs_counts = preprocess_dataset(
        input_dir=config.VIIRS_FIRE_DIR,
        output_dir=viirs_out,
        dataset_name="VIIRS",
        logger=logger,
    )
    
    total_processed = modis_counts["processed"] + viirs_counts["processed"]
    total_skipped = modis_counts["skipped"] + viirs_counts["skipped"]
    total_failed = modis_counts["failed"] + viirs_counts["failed"]
    ...
    summary = (
        "\n==================================\n"
        "Fire Preprocessing Summary\n"
        "==================================\n"
        f"MODIS processed: {modis_counts['processed']}\n"
        f"MODIS skipped: {modis_counts['skipped']}\n"
        f"MODIS failed: {modis_counts['failed']}\n\n"
        f"VIIRS processed: {viirs_counts['processed']}\n"
        f"VIIRS skipped: {viirs_counts['skipped']}\n"
        f"VIIRS failed: {viirs_counts['failed']}\n\n"
